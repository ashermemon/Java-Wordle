...
public class Wordle { //Creates a class containing the app
    ...
    public static void CreateKeyboard(String[] guesses, char[] yellows, char[] greens)//Defining a method that creates a visible keyboard in the terminal
    ...
    public static void printGuesses(String[] guesses){ //defines a method to print the guesses with a slight delay between each one (for the start of the game)
    ...
    public static void playGame (Scanner input) throws IOException { //defines main game loop, takes in the scanner
            ...
            String[] guesses = new String[6]; //creates an array of strings; one string for each guess
            ...
            for(int j =0; j<guesses.length && !won; j++){ //loop which runs for each guess
                ...
                while(guesses[j].length() != 5 || count != 5 || isRealWord(guesses[j]) == false){
                    ...
                    guesses[j] = input.next().trim(); //Waits for a user input
                    input.nextLine(); 
                    

                    
                    count = guesses[j].replaceAll("[^A-Za-z]", "").length(); //counts all the a to z or A to Z characters
                
                    System.out.print("\033[1A\033[K"); //Delete last line
                    if (!guesses[j].contains(" ") && guesses[j].length() == 5 && count == 5 && isRealWord(guesses[j]) == true) break; //if the word doesnt have a space, symbols, and is a real word, then break
                ...
                guesses[j] = guesses[j].toUpperCase(); //make user word uppercase to match the case of the target word

                if(guesses[j].equals(target_word)) // check if the guess is the same as the target
                {
                ...
                for(int i=0; i<=j; i++){ //Colouring logic plays after the guess, reprinting all previous guesses
                    ...
                    for(int x=0; x<guesses[i].length(); x++)
                    {
                        if(guesses[i].charAt(x) == targetCopy[x]) //if the letter in the guess is in the same place in the target word
                        {
                            ...
                            System.out.print(GREEN + guesses[i].charAt(x) + RESET + " "); //Print the letter as green
                            ...
                            if (index >= 0){ //if the word contains the letter
                                ...
                                System.out.print(YELLOW + guesses[i].charAt(x) + RESET  + " "); //Print the letter as yellow
                                ...
                                System.out.print(guesses[i].charAt(x)  + " "); //else print it as normal (no effects)
                            }
